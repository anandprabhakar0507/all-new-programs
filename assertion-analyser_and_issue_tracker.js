...
function replacer(str) {
  ...
  while ((obj = objParser(str))) {
    data[cnt] = obj.obj;
    str = str.substring(0, obj.start) + "__#" + cnt++ +
...
function assertionAnalyser(body) {
  ...
  let cleanedBody =
      body.match(/(?:browser\s*\.\s*)?assert\s*\.\s*\w*\([\s\S]*\)/);
  ...
  let bodies = assertionBodies.map(function(
      b) { return s.dictionary[b].slice(1, -1).trim(); });
